...
def categorize_dish(dish_name, dish_ingredients):
    ...
    cat_to_text = {
         ...
         id(PALEO):'PALEO',
         id(KETO):'KETO',
         id(OMNIVORE):'OMNIVORE'
    }
    cats = [VEGAN, VEGETARIAN, PALEO, KETO, OMNIVORE]
    for cat in cats:
        if dish_ingredients <= cat:
            return dish_name + ': ' + cat_to_text[id(cat)]
    return '<NO_CATEGORY>'
...
def tag_special_ingredients(dish):
    ...
    """
    name,ingr = dish
    return (name,set(ingr) & set(SPECIAL_INGREDIENTS))
